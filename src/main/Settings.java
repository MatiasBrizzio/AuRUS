...
public class Settings {
    ...
    /**
     * The seed backing {@link #RANDOM_GENERATOR}. Auto-generated by default
     * (so runs are non-deterministic out of the box) from the system clock,
     * mixed with the 64-bit golden-ratio constant ({@code 0x9E3779B97F4A7C15})
     * for good bit dispersion (the standard Fibonacci/golden-ratio hashing
     * trick — avoids the low bits of consecutive {@link System#nanoTime()}
     * calls correlating between close-together runs). Always a concrete
     * value — printed by {@link #print_settings()} in every {@code out.txt}
     * — so any past run can be reproduced after the fact by passing
     * {@code -seed=<that value>}, even if it wasn't explicitly seeded when
     * it originally ran. Explicitly setting it (flag {@code -seed}) makes
     * AuRUS's own stochastic decisions (selection, mutation targets,
     * crossover choices) reproducible given the same sequence of external
     * solver verdicts — see the project {@code TODO.md} for the current,
     * honest state of end-to-end reproducibility (not yet complete: at
     * least one additional source of run-to-run variance remains,
     * independent of this seed).
     */
    public static long SEED = System.nanoTime() * 0x9E3779B97F4A7C15L;

    /** Shared random source for every stochastic decision in the search (selection coin flips, mutation targets, ...), seeded from {@link #SEED}. Reproducible across runs when {@link #SEED} is set explicitly via {@code -seed}. */
    public static Random RANDOM_GENERATOR = new Random(SEED);

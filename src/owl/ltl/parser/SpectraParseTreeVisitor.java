...
final class SpectraParseTreeVisitor extends SPECTRAParserBaseVisitor<Formula> {
  ...
  public Formula visitPredPatt(PredPattContext ctx) {
	...
	if (op.contentEquals("notP_globally")) {
		Formula operand = ctx.temporalExpr().get(0).accept(this);
		return GOperator.of(operand.not());
	} else if (op.contentEquals("pRespondsToS")) {
		Formula left = ctx.temporalExpr().get(0).accept(this);
		Formula right = ctx.temporalExpr().get(1).accept(this);
		return GOperator.of(Disjunction.of(left.not(), FOperator.of(right)));
	}
	else if (op.contentEquals("notP_beforeR")) {
		Formula left = ctx.temporalExpr().get(0).accept(this);
		Formula right = ctx.temporalExpr().get(1).accept(this);
		return Disjunction.of(UOperator.of(left.not(),right), FOperator.of(right).not());
	}
	else if (op.contentEquals("pBecomesTrue_betweenQandR")) { //[](Q & !R -> (!R W (P & !R)))
		Formula p = ctx.temporalExpr().get(0).accept(this);
		Formula q = ctx.temporalExpr().get(1).accept(this);
		Formula r = ctx.temporalExpr().get(1).accept(this);
		return GOperator.of(Disjunction.of(Conjunction.of(q, r.not()).not(), WOperator.of(r.not(), Conjunction.of(p, r.not()))));
	}
//	else if (op.contentEquals("pHolds_afterQuntilR")) { 
//		//[](Q -> ((!P & !R) U (R | ((P & !R) U
//        //          (R | ((!P & !R) U (R | ((P & !R) U
//        //               (R | (!P W R) | []P)))))))))
//		Formula p = ctx.temporalExpr().get(0).accept(this);
